...
export default {
  ...
  computed: {
    ...
    filteredProducts() {
    return this.romanceCollection.filter((item)=>{
  ...
  methods: {
    ...
    async confirmUpdate(itemForUpdate) {
      await this.$store.dispatch("updateRomanceProduct", this.itemForUpdate);
    ...
    async confirmAdd(){
      ...
      await this.$store.dispatch('createRomanceProduct' , romanceProduct );
      this.$store.dispatch('getRomanceCollection');
