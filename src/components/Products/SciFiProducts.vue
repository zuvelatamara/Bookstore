...
export default {
  ...
  computed: {
    ...mapState(["scifiCollection"]),
    ...mapActions(["getSciFiCollection"]),
    filteredProducts() {
    return this.scifiCollection.filter((item)=>{
  ...
  // LIFE-CYCLE hooks
  mounted() {
    this.$store.dispatch("getSciFiCollection");
  },
  methods: {
    onFilePicked(event){
    ...
    confirmDelete(id) {
      this.$store.dispatch("deleteSciFiProduct", id);
      this.closeDeleteModal();
      this.$store.dispatch("getSciFiCollection");
    },
    async confirmUpdate(itemForUpdate) {
      await this.$store.dispatch("updateSciFiProduct", this.itemForUpdate);
    ...
    async confirmAdd(){
      ...
      await this.$store.dispatch('createSciFiProduct' , scifiProduct );
      this.$store.dispatch('getSciFiCollection');
