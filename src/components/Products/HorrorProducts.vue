...
export default {
  ...
  computed: {
    ...mapState(["horrorCollection"]),
    ...mapActions(["getHorrorCollection"]),
    filteredProducts() {
    return this.horrorCollection.filter((item)=>{
  ...
  // LIFE-CYCLE hooks
  mounted() {
    this.$store.dispatch("getHorrorCollection");
  },
  methods: {
    onFilePicked(event){
    ...
    confirmDelete(id) {
      this.$store.dispatch("deleteHorrorProduct", id);
      this.closeDeleteModal();
      this.$store.dispatch("getHorrorCollection");
    },
    async confirmUpdate(itemForUpdate) {
      await this.$store.dispatch("updateHorrorProduct", this.itemForUpdate);
    ...
    async confirmAdd(){
      ...
      await this.$store.dispatch('createHorrorProduct' , horrorProduct );
      this.$store.dispatch('getHorrorCollection');
